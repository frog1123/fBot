...
function loadBot(num) {
    ...
    const bot = config.bots[num];

    const prefix = bot.prefix;

    client.commands = new Discord.Collection();

    const color = chalk.hex(`#${bot.color}`);
    ...
    client.once('ready', () => {
        console.log(`${color(client.user.tag)} is online`);
        ...
        client.user.setPresence({ status: bot.status });
    ...
    client.on('messageCreate', message => {
        ...
        if (typeof client.commands.get(command) !== 'undefined') {
            client.commands.get(command).execute(message, args, Discord, client, num);
        }
    });
